...
static int append_usage_synopsis(ap_string_builder *sb, const ap_parser *parser) {
  int i;
...
static int append_option_signature(ap_string_builder *sb, const ap_arg_def *def) {
  int i;
...
static int append_choices_line(ap_string_builder *sb, const ap_choices *choices) {
  int i;
...
static int append_options_section(ap_string_builder *sb, const ap_parser *parser,
                                  bool *has_options) {
  int i;
  bool emitted = false;
  ...
  for (i = 0; i < parser->defs_count; i++) {
    ...
    if (append_choices_line(sb, &def->opts.choices) != 0 ||
        ...
        (def->opts.required &&
         append_field_line(sb, "required", "yes") != 0)) {
      return -1;
    }
    if (ap_sb_appendf(sb, "\n") != 0) {
      return -1;
...
char *ap_manpage_build(const ap_parser *parser) {
  ...
  } else if (ap_sb_appendf(&sb,
                           "Command line interface generated by argparse\\-c.") !=
             0) {
    ap_sb_free(&sb);
    return NULL;
  }
  ...
  if (ap_sb_appendf(&sb,
                    ...
                    ".SH ERRORS\n"
                    "Typical failures include unknown options, missing values, invalid choices, required argument violations, and unexpected positional arguments.\n") != 0) {
    ap_sb_free(&sb);
    return NULL;
  }
