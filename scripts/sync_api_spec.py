...
def to_spec(functions: list[Function]) -> dict:
    return {
        ...
        "error_model": {
            "error_type": "ap_error",
            "code_enum_type": "ap_error_code",
            "fields": [
                {"name": "code", "type": "ap_error_code"},
                {"name": "argument", "type": "char[64]"},
                {"name": "message", "type": "char[256]"},
            ],
            "public_codes": ERROR_CODE_ENUM_VALUES,
            "backward_compatibility": {
                "en": "Clients should key logic on `ap_error.code`; `message` remains human-readable and not a stable machine contract.",
                "ja": "クライアントは `ap_error.code` を判定に使い、`message` は人間向け表示（機械判定の安定契約ではない）として扱ってください。",
            },
        },
...
def render_markdown(spec: dict, lang: str) -> str:
    ...
    lines = [AUTOGEN_COMMENT.rstrip(), f"# {title}", "", intro, ""]
    ...
    if error_model:
        if is_ja:
            lines.extend(
                [
                    "## エラーモデル",
                    f"- エラー構造体: `{error_model.get('error_type', 'ap_error')}`",
                    f"- 公開コード enum: `{error_model.get('code_enum_type', 'ap_error_code')}`",
                    "- フィールド:",
                ]
            )
            for field in error_model.get("fields", []):
                lines.append(f"  - `{field['name']}`: `{field['type']}`")
            lines.append("- 公開エラーコード:")
            for code in error_model.get("public_codes", []):
                lines.append(f"  - `{code}`")
            lines.append(
                f"- 後方互換方針: {error_model.get('backward_compatibility', {}).get('ja', '')}"
            )
            lines.append("")
        else:
            lines.extend(
                [
                    "## Error model",
                    f"- Error struct: `{error_model.get('error_type', 'ap_error')}`",
                    f"- Public code enum: `{error_model.get('code_enum_type', 'ap_error_code')}`",
                    "- Fields:",
                ]
            )
            for field in error_model.get("fields", []):
                lines.append(f"  - `{field['name']}`: `{field['type']}`")
            lines.append("- Public error codes:")
            for code in error_model.get("public_codes", []):
                lines.append(f"  - `{code}`")
            lines.append(
                f"- Backward compatibility policy: {error_model.get('backward_compatibility', {}).get('en', '')}"
            )
            lines.append("")
